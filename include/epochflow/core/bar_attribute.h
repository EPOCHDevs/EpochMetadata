...
struct EpochStratifyXConstants {
  ...
  const epochflow::TimeFrame DAILY_FREQUENCY{
      epoch_frame::factory::offset::days(1)};
  const epochflow::TimeFrame MINUTE_FREQUENCY{
...
struct BarsConstants {
  ...
  const std::vector<std::string> all{
      ...
      epochflow::EpochStratifyXConstants::instance().VOLUME()};

  const std::vector<epochflow::BarAttribute> all_attributes{
      epochflow::EpochStratifyXConstants::instance().OPEN,
      epochflow::EpochStratifyXConstants::instance().CLOSE,
      epochflow::EpochStratifyXConstants::instance().HIGH,
      epochflow::EpochStratifyXConstants::instance().LOW,
  ...
  const arrow::FieldVector all_fields{
      arrow::field(epochflow::EpochStratifyXConstants::instance().OPEN(),
                   arrow::float64()),
      arrow::field(epochflow::EpochStratifyXConstants::instance().HIGH(),
                   arrow::float64()),
      arrow::field(epochflow::EpochStratifyXConstants::instance().LOW(),
                   arrow::float64()),
      arrow::field(epochflow::EpochStratifyXConstants::instance().CLOSE(),
                   arrow::float64()),
      arrow::field(epochflow::EpochStratifyXConstants::instance().VOLUME(),
                   arrow::float64()),
  };
};
...
struct FuturesConstants {
  ...
  const std::vector<std::string> all = epoch_frame::chain(
      BarsConstants{}.all,
      std::vector{
          epochflow::EpochStratifyXConstants::instance().CONTRACT(),
          epochflow::EpochStratifyXConstants::instance().OPEN_INTEREST()});

  const arrow::FieldVector all_fields = epoch_frame::chain(
      BarsConstants{}.all_fields,
      std::vector{
          field(epochflow::EpochStratifyXConstants::instance().CONTRACT(),
                arrow::utf8()),
          field(epochflow::EpochStratifyXConstants::instance()
                    .OPEN_INTEREST(),
                arrow::float64())});
...
struct OptionsConstants {
  ...
  const std::vector<std::string> all = epoch_core::merge(
      std::vector{epochflow::EpochStratifyXConstants::instance().PRICE(),
                  epochflow::EpochStratifyXConstants::instance().IV(),
                  epochflow::EpochStratifyXConstants::instance().DELTA(),
                  epochflow::EpochStratifyXConstants::instance().GAMMA(),
                  epochflow::EpochStratifyXConstants::instance().VEGA(),
                  epochflow::EpochStratifyXConstants::instance().THETA(),
                  epochflow::EpochStratifyXConstants::instance().RHO()},
      FuturesConstants{}.all);
};

struct QuotesConstants {
  const std::vector<std::string> all{
      epochflow::EpochStratifyXConstants::instance().ASK(),
      epochflow::EpochStratifyXConstants::instance().ASK_VOLUME(),
      epochflow::EpochStratifyXConstants::instance().BID(),
      epochflow::EpochStratifyXConstants::instance().BID_VOLUME()};
};

struct TradesConstants {
  const std::vector<std::string> all{
      epochflow::EpochStratifyXConstants::instance().PRICE(),
      epochflow::EpochStratifyXConstants::instance().VOLUME()};
};
