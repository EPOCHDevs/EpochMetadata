...
{
  template <>
  struct convert<epochflow::MetaDataOption>
  {
    static bool decode(const Node &node, epochflow::MetaDataOption &t)
    {
      t.decode(node);
      return true;
    }
  ...
  template <>
  struct convert<epochflow::SelectOption>
  {
    static bool decode(const Node &node, epochflow::SelectOption &t)
    {
      t.decode(node);
      return true;
    }
...
{
  template <>
  struct meta<epochflow::MetaDataArgRef>
  {
    using T = epochflow::MetaDataArgRef;
    static constexpr auto value = object("refName", &T::refName);
  };

  // SqlStatement serialization - serialize/deserialize as plain string
  template <>
  struct meta<epochflow::SqlStatement>
  {
    static constexpr auto read =
        [](epochflow::SqlStatement &value, const std::string &in)
    {
      value = epochflow::SqlStatement{in};
    };

    static constexpr auto write =
        [](const epochflow::SqlStatement &x) -> std::string
    {
      return x.GetSql();
    };
  ...
  template <>
  struct meta<epochflow::MetaDataOptionDefinition>
  {
    static constexpr auto read =
        [](epochflow::MetaDataOptionDefinition &value, const generic &in)
    {
      if (in.is_number())
      {
        value = epochflow::MetaDataOptionDefinition{in.get<double>()};
      }
      else if (in.is_boolean())
      {
        value = epochflow::MetaDataOptionDefinition{in.get<bool>()};
      }
      else if (in.is_string())
      {
        value =
            epochflow::MetaDataOptionDefinition{in.get<std::string>()};
      }
      else if (in.is_object() && in.contains("refName"))
      {
        auto refName = in["refName"].get<std::string>();
        value = epochflow::MetaDataOptionDefinition{
            epochflow::MetaDataArgRef{refName}};
      }
      else if (in.is_object() && in.contains("hour") && in.contains("minute"))
      {
        ...
        value = epochflow::MetaDataOptionDefinition{
            epochflow::MetaDataOptionDefinition::T{time}};
      }
      else if (in.is_object() && in.contains("schemas"))
      {
        ...
        {
          // CardSchemaFilter
          epochflow::CardSchemaFilter schema;
          ...
          value = epochflow::MetaDataOptionDefinition{
              epochflow::MetaDataOptionDefinition::T{std::move(schema)}};
        }
        else if (in.contains("sql"))
        {
          // CardSchemaSQL
          epochflow::CardSchemaSQL schema;
          auto error = glz::read_json(schema, in.dump().value_or("{}"));
          if (error)
          {
      ...
      else
      {
        ...
        value = epochflow::MetaDataOptionDefinition{dumped.value()};
      }
    };

    static constexpr auto write =
        [](const epochflow::MetaDataOptionDefinition &x) -> auto
    {
      return x.GetVariant();
    };
