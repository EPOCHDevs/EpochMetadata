...
struct DateOffsetOption
{
  ...
  // Session-anchored `
  std::optional<epochflow::strategy::SessionVariant> session{std::nullopt};
...
namespace glz
{
  ...
  struct meta<epoch_frame::DateOffsetHandlerPtr>
  {
    static constexpr auto read = [](epoch_frame::DateOffsetHandlerPtr &x,
                                    const generic &input)
    {
      x = epochflow::CreateDateOffsetHandlerFromJSON(input);
    };

    static constexpr auto write =
        [](const epoch_frame::DateOffsetHandlerPtr &x) -> auto
    {
      return epochflow::CreateDateOffsetHandlerJSON(x);
    };
  ...
  template <>
  struct meta<epochflow::TimeFrame>
  {
    static constexpr auto read = [](epochflow::TimeFrame &x,
                                    const glz::generic &input)
    {
      if (input.is_string())
      {
        x = epochflow::TimeFrame(input.as<std::string>());
        return;
      }
      auto offset = epochflow::CreateDateOffsetHandlerFromJSON(input);
      if (offset)
      {
        x = epochflow::TimeFrame(offset);
      }
    };

    static constexpr auto write = [](const epochflow::TimeFrame &x) -> auto
    {
      ...
      return epochflow::CreateDateOffsetHandlerJSON(x.GetOffset());
    };
  ...
  template <>
  struct meta<std::optional<epochflow::TimeFrame>>
  {
    static constexpr auto read = [](std::optional<epochflow::TimeFrame> &x,
                                    const glz::generic &input)
    {
      if (input.is_null())
      {
    ...
    static constexpr auto write =
        [](const std::optional<epochflow::TimeFrame> &x) -> auto
    {
      if (x)
      {
        return epochflow::CreateDateOffsetHandlerJSON(x->GetOffset());
...
namespace YAML
{
  ...
  template <>
  struct convert<epochflow::TimeFrame>
  {
    static bool decode(const Node &node, epochflow::TimeFrame &rhs)
    {
      if (node.IsScalar())
      {
        rhs = epochflow::TimeFrame(node.as<std::string>());
        return true;
      }
      rhs =
          epochflow::TimeFrame(node.as<epoch_frame::DateOffsetHandlerPtr>());
      return true;
    }
  };
} // namespace YAML