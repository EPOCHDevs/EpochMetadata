...
{
  template <>
  struct convert<epochflow::strategy::SessionVariant>
  {
    static bool decode(const Node &node,
                       epochflow::strategy::SessionVariant &out);
  };

  template <>
  struct convert<epochflow::strategy::AlgorithmNode>
  {
    static bool decode(YAML::Node const &,
                       epochflow::strategy::AlgorithmNode &);
  };

  template <>
  struct convert<epochflow::strategy::AlgorithmBaseMetaData>
  {
    static bool decode(YAML::Node const &,
                       epochflow::strategy::AlgorithmBaseMetaData &);
  };

  template <>
  struct convert<epochflow::strategy::AlgorithmMetaData>
  {
    static bool decode(YAML::Node const &,
                       epochflow::strategy::AlgorithmMetaData &);
  };

  epochflow::strategy::TradeSignalMetaData decode(glz::generic const &);

  glz::generic encode(epochflow::strategy::TradeSignalMetaData const &);
...
namespace glz
{
  ...
  template <>
  struct to<JSON, epochflow::strategy::PythonSource>
  {
    template <auto Opts>
    static void op(const epochflow::strategy::PythonSource &x, auto &&...args) noexcept
    {
  ...
  template <>
  struct from<JSON, epochflow::strategy::PythonSource>
  {
    template <auto Opts>
    static void op(epochflow::strategy::PythonSource &value, auto &&...args)
    {
      std::string source_code;
      parse<JSON>::op<Opts>(source_code, args...);
      value = epochflow::strategy::PythonSource(source_code);
    }
  };
} // namespace glz