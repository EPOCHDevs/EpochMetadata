...
namespace epochflow::transform {
  ...
  struct SelectorData {
    ...
    std::vector<epochflow::CardColumnSchema> schemas;
    ...
    SelectorData(std::string title_,
                 std::vector<epochflow::CardColumnSchema> schemas_,
...
struct ITransformBase {
  ...
  virtual epochflow::MetaDataOptionDefinition
  GetOption(std::string const &param) const = 0;

  virtual epochflow::MetaDataOptionList GetOptionsMetaData() const = 0;
  ...
  virtual std::vector<epochflow::transforms::IOMetaData>
  GetOutputMetaData() const = 0;

  virtual epochflow::TimeFrame GetTimeframe() const = 0;
...
public:
  ...
  epochflow::MetaDataOptionList GetOptionsMetaData() const final {
  ...
  std::vector<std::string> GetInputIds() const override {
    ...
    std::ranges::for_each(
        inputs, [&](epochflow::transforms::IOMetaData const &io) {
          auto out = m_config.GetInputs(io.id);
          if (out.empty()) {
            AssertFromStream(
                m_config.GetTransformName() ==
                    epochflow::transforms::TRADE_SIGNAL_EXECUTOR_ID,
  ...
  std::vector<epochflow::transforms::IOMetaData>
  GetOutputMetaData() const override {
    return m_config.GetOutputs();
  }

  epochflow::TimeFrame GetTimeframe() const final {
