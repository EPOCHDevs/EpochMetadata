...
class IReporter : public epochflow::transform::ITransform {
public:
  explicit IReporter(epochflow::transform::TransformConfiguration config, bool skipRename=false)
...
void RegisterReport() {
  ...
  epochflow::transforms::ITransformRegistry::GetInstance().Register(metadata);

  // 2. Register the transform factory using metadata.id
  epochflow::transform::Register<ReportClass>(metadata.id);
}
