...
struct DateOffsetOption
{
  ...
  // Session-anchored `
  std::optional<epoch_script::strategy::SessionVariant> session{std::nullopt};
...
namespace glz
{
  ...
  struct meta<epoch_frame::DateOffsetHandlerPtr>
  {
    static constexpr auto read = [](epoch_frame::DateOffsetHandlerPtr &x,
                                    const generic &input)
    {
      x = epoch_script::CreateDateOffsetHandlerFromJSON(input);
    };

    static constexpr auto write =
        [](const epoch_frame::DateOffsetHandlerPtr &x) -> auto
    {
      return epoch_script::CreateDateOffsetHandlerJSON(x);
    };
  ...
  template <>
  struct meta<epoch_script::TimeFrame>
  {
    static constexpr auto read = [](epoch_script::TimeFrame &x,
                                    const glz::generic &input)
    {
      if (input.is_string())
      {
        x = epoch_script::TimeFrame(input.as<std::string>());
        return;
      }
      auto offset = epoch_script::CreateDateOffsetHandlerFromJSON(input);
      if (offset)
      {
        x = epoch_script::TimeFrame(offset);
      }
    };

    static constexpr auto write = [](const epoch_script::TimeFrame &x) -> auto
    {
      ...
      return epoch_script::CreateDateOffsetHandlerJSON(x.GetOffset());
    };
  ...
  template <>
  struct meta<std::optional<epoch_script::TimeFrame>>
  {
    static constexpr auto read = [](std::optional<epoch_script::TimeFrame> &x,
                                    const glz::generic &input)
    {
      if (input.is_null())
      {
    ...
    static constexpr auto write =
        [](const std::optional<epoch_script::TimeFrame> &x) -> auto
    {
      if (x)
      {
        return epoch_script::CreateDateOffsetHandlerJSON(x->GetOffset());
...
namespace YAML
{
  ...
  template <>
  struct convert<epoch_script::TimeFrame>
  {
    static bool decode(const Node &node, epoch_script::TimeFrame &rhs)
    {
      if (node.IsScalar())
      {
        rhs = epoch_script::TimeFrame(node.as<std::string>());
        return true;
      }
      rhs =
          epoch_script::TimeFrame(node.as<epoch_frame::DateOffsetHandlerPtr>());
      return true;
    }
  };
} // namespace YAML