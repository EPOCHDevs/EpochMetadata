...
using Column = std::vector<std::optional<Value>>;
...
struct TestCase {
    ...
    Options options;
    std::vector<std::string> timestamp_columns;  // Columns to convert to timestamps
    std::string index_column;                     // Column to use as index (optional)
};
...
protected:
    ...
    static std::optional<Value> parseValue(const YAML::Node& node);
