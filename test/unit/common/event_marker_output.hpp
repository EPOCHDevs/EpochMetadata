...
public:
    ...
    bool equals(const IOutputType& other) const override {
        ...
        return compareEventMarkerData(selectorData, otherSelector->selectorData);
    }
...
private:
    bool compareEventMarkerData(const epoch_script::transform::EventMarkerData& a,
                            const epoch_script::transform::EventMarkerData& b) const {
        // Compare title
        if (a.title != b.title) {
            std::cerr << "DEBUG compareEventMarkerData: Title mismatch - a='" << a.title
                      << "', b='" << b.title << "'" << std::endl;
            return false;
        }
        ...
        // Compare icon
        if (a.icon != b.icon) {
            std::cerr << "DEBUG compareEventMarkerData: Icon mismatch - a=" << static_cast<int>(a.icon)
        ...
        if (a.schemas.size() != b.schemas.size()) {
            std::cerr << "DEBUG compareEventMarkerData: Schema count mismatch - a=" << a.schemas.size()
        ...
        if (a.pivot_index != b.pivot_index) {
            std::cerr << "DEBUG compareEventMarkerData: Pivot index mismatch - a=";
        ...
        if (b.data.num_rows() > 0 || b.data.num_cols() > 0) {
            ...
            if (a.data.num_rows() != b.data.num_rows()) {
                std::cerr << "DEBUG compareEventMarkerData: DataFrame row count mismatch - a=" << a.data.num_rows()
                          << ", b=" << b.data.num_rows() << std::endl;
                return false;
            }

            if (a.data.num_cols() != b.data.num_cols()) {
                std::cerr << "DEBUG compareEventMarkerData: DataFrame col count mismatch - a=" << a.data.num_cols()
            ...
            if (a_cols_set != b_cols_set) {
                std::cerr << "DEBUG compareEventMarkerData: DataFrame column names mismatch" << std::endl;
            ...
            for (const auto& colName : b_cols) {
                try {
                    ...
                    if (!a_col.equals(b_col)) {
                        std::cerr << "DEBUG compareEventMarkerData: Column '" << colName << "' data does not match" << std::endl;
                        std::cerr << "Actual column:\n" << a_col << std::endl;
                        std::cerr << "Expected column:\n" << b_col << std::endl;
                        return false;
                    }
                } catch (const std::exception& e) {
                    std::cerr << "DEBUG compareEventMarkerData: Error comparing column '" << colName << "': " << e.what() << std::endl;
                    return false;
                }
            }
        }
