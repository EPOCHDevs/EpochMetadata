 ...
 * @endcode
 */
class MockTransform : public epoch_metadata::transform::ITransformBase {
public:
    ...
    MAKE_CONST_MOCK0(GetSelectorData, epoch_metadata::transform::SelectorData(), override);
    ...
    [[nodiscard]] epoch_metadata::TimeFrame GetTimeframe() const override { return *m_timeframe; }
    [[nodiscard]] std::vector<std::string> GetInputIds() const override { return m_inputIds; }
    [[nodiscard]] std::vector<epoch_metadata::transforms::IOMetaData> GetOutputMetaData() const override {
    ...
    [[nodiscard]] epoch_metadata::MetaDataOptionDefinition GetOption(std::string const&) const override {
        throw std::runtime_error("GetOption() not implemented in MockTransform stub");
    }
    [[nodiscard]] epoch_metadata::MetaDataOptionList GetOptionsMetaData() const override {
        return {};  // Return empty list
    }
    [[nodiscard]] epoch_metadata::transform::TransformConfiguration GetConfiguration() const override {
        ...
        if (!m_cachedConfig) {
            ...
            m_cachedConfig = std::make_unique<epoch_metadata::transform::TransformConfiguration>(
                epoch_metadata::TransformDefinition{YAML::Load(yamlStr)}
            );
        }
        return *m_cachedConfig;
    }

    // Storage for expectations (only for TransformData and GetTearSheet)
    ...
    std::optional<epoch_metadata::TimeFrame> m_timeframe;  // Optional since TimeFrame has no default ctor
    std::vector<std::string> m_inputIds;
    std::vector<std::string> m_outputIds;
    std::vector<epoch_metadata::transforms::IOMetaData> m_outputMetadata;
    ...
    mutable std::unique_ptr<epoch_metadata::transform::TransformConfiguration> m_cachedConfig;  // Lazy-initialized config
};
