...
epoch_metadata::transform::TransformConfigurationList loadConfigurationFromYAML(const YAML::Node& node) {
    epoch_metadata::transform::TransformConfigurationList configurations;
    ...
    for (const auto& transformNode : node["transforms"]) {
        ...
        epoch_metadata::TransformDefinition definition(transformNode);
        epoch_metadata::transform::TransformConfiguration config(std::move(definition));
