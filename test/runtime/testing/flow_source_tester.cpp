...
void FlowSourceTestRunner::RunTestCase(const TestCase& testCase, bool updateMode) {
    ...
    std::optional<epoch_metadata::TimeFrame> baseTimeframe;
    if (!testCase.config.timeframes.empty()) {
        baseTimeframe = epoch_metadata::TimeFrame{testCase.config.timeframes[0]};
    } else {
        // Use first timeframe from input data
        if (!inputData.empty()) {
            baseTimeframe = epoch_metadata::TimeFrame{inputData.begin()->first};
        }
    }
...
epoch_metadata::strategy::PythonSource
FlowSourceTestRunner::LoadSource(const std::filesystem::path& sourceFile) {
    ...
    // Compile source
    return epoch_metadata::strategy::PythonSource(sourceCode);
}

std::vector<std::string> FlowSourceTestRunner::DetectRequiredAssets(
    const epoch_metadata::strategy::PythonSource& source,
