...
static std::string valueToString(const std::optional<Value>& v) {
    if (!v) {
        return "null";
    }
...
bool DataFrameOutput::equals(const IOutputType& other) const {
    ...
    for (const auto& [colName, column] : data) {
        ...
        for (size_t i = 0; i < column.size(); ++i) {
            if (!column[i] && !otherCol[i]) {
                continue;
            }
            if (column[i].has_value() != otherCol[i].has_value()) {
                return false;
            }
            ...
            if (std::holds_alternative<double>(*column[i])) {
                ...
                double tolerance = 1e-3;  // 0.1% relative tolerance for 4 decimal places
            ...
            } else if (column[i].value() != otherCol[i].value()) {
                return false;
            }
        }
    }

    // Also check that otherDf doesn't have extra columns
    for (const auto& [colName, column] : otherDf.data) {
