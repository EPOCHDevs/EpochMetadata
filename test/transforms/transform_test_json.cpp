    ...
    SECTION(sectionName) {
        ...
        for (const auto& jsonTest : jsonTests) {
            SECTION(jsonTest.title) {
                ...
                // Normalize column names in input (replace # with _) for SQL compatibility
                Table normalizedInput;
                for (const auto& [colName, colData] : testCase.input) {
                    std::string normalizedName = colName;
                    std::replace(normalizedName.begin(), normalizedName.end(), '#', '_');
                    normalizedInput[normalizedName] = colData;
                }

                // Also normalize the inputs SLOT if present
                auto inputsIt = testCase.options.find("inputs");
                if (inputsIt != testCase.options.end() && std::holds_alternative<std::string>(inputsIt->second)) {
                    std::string inputsStr = std::get<std::string>(inputsIt->second);
                    // Replace # with _ in the inputs string
                    std::replace(inputsStr.begin(), inputsStr.end(), '#', '_');
                    testCase.options["inputs"] = inputsStr;
                }
                ...
                epoch_frame::DataFrame inputDf = CatchTransformTester::tableToDataFrame(
                    normalizedInput, testCase.timestamp_columns, testCase.index_column);
