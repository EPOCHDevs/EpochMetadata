...
DataFrameTransformTester::TestCaseType convertJsonToTestCase(const json::TestCase& jsonTest) {
    ...
    if (jsonTest.expect.has_value()) {
        ...
        if (std::holds_alternative<TearsheetExpect>(expectVar)) {
            ...
            // Charts are handled differently - they are generated by the chart builders
            // and added to the tearsheet automatically during report execution.
            // For now, we'll skip manual chart construction in tests and rely on
            // the report implementation to generate charts that we can compare.
