...
MakeConfig(std::string const &id, epoch_metadata::TimeFrame const &tf,
           std::optional<epoch_frame::SessionRange> sessionRange,
           bool intradayOnly) {
  epoch_metadata::transforms::TransformsMetaData meta{};
  meta.id = id;
...
TEST_CASE("requiresSession slices to provided SessionRange",
  ...
  auto tf =
      epoch_metadata::TimeFrame(epoch_frame::factory::offset::minutes(30));
