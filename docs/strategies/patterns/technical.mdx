...
<Tabs items={['Basic', 'Enhanced']}>
  ...
  <Tab value="Enhanced">
    ...
    # Enhanced signals
    buy_confirmed = crossover(fast, slow) and volume_confirm and trending
    sell_confirmed = crossover(slow, fast) and volume_confirm and trending
...
<Tabs items={['Basic', 'With Trend Filter']}>
  <Tab value="Basic">
    ```python
    src = market_data_source()
...
<Callout type="tip">
Multi-timeframe analysis provides better context and reduces false signals by ensuring all timeframes are aligned.
</Callout>
