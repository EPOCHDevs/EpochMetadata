---
title: EpochScript Design Guidelines
page_type: concept
layout: default
order: 4
category: Language
description: Essential design patterns, executor requirements, Python incompatibilities, and common mistakes
parent: ./index.md
---

import { Callout } from 'fumadocs-ui/components/callout';

# EpochScript Design Guidelines

Essential design patterns and constraints for writing EpochScript strategies.

---


## Strategy Executor Requirements

### Every Trading Strategy Must Have Exactly One Executor

Trading strategies require exactly one `trade_signal_executor()` call. This is not an error you encounter randomly - it's a fundamental design requirement.

**Why?** The executor is the bridge between your strategy logic and actual trade execution. Without it, your strategy is just calculations with no trades.

#### Pattern: Single Executor

<!-- snippet: type=complete_code, needs_data=single_asset, timeframe=1D, complexity=medium -->
```python
# Correct: - One executor with all signals
src = market_data_source(timeframe="1D")

fast = ema(period=12)(src.c)
slow = ema(period=26)(src.c)

buy_signal = crossover(fast, slow)
sell_signal = crossover(slow, fast)

trade_signal_executor()(
    enter_long=buy_signal,
    enter_short=sell_signal
)
```

#### Anti-Pattern: Missing Executor

<!-- snippet: type=error_example -->
```python
# Incorrect: - No executor
src = market_data_source(timeframe="1D")
signal = src.c > src.c[1]
# Strategy ends here - no trades will execute!
```

**Error message:** `No trade_signal_executor found in strategy`

#### Anti-Pattern: Multiple Executors

<!-- snippet: type=error_example -->
```python
# Incorrect: - Multiple executors (conflicting logic)
trade_signal_executor()(enter_long=buy)
trade_signal_executor()(enter_short=sell)  # Second executor conflicts!
```

**Error message:** `Multiple trade_signal_executor calls found`

**Fix:** Combine all signals into single executor call.

#### Anti-Pattern: Empty Executor

<!-- snippet: type=error_example -->
```python
# Incorrect: - Executor with no signals
trade_signal_executor()()  # What should it execute?
```

**Error message:** `trade_signal_executor requires at least one input`

**Fix:** Provide at least one signal (enter_long, enter_short, exit_long, or exit_short).

### Executor Signal Inputs

The executor accepts four optional boolean inputs (but requires at least one):

- `enter_long` - Signal to open long position
- `enter_short` - Signal to open short position
- `exit_long` - Signal to close long position
- `exit_short` - Signal to close short position

**Execution order:** Exits are processed BEFORE entries on the same bar.

**Example: Long-only strategy with exit logic**

<!-- snippet: type=complete_code, needs_data=single_asset, timeframe=1D, complexity=medium -->
```python
src = market_data_source(timeframe="1D")
ema_20 = ema(period=20)(src.c)
atr_val = atr(period=14)(src.h, src.l, src.c)

# Entry signal
buy = crossover(src.c, ema_20)

# Exit signal (stop-loss or take-profit)
stop_distance = 2 * atr_val
exit = src.c < (ema_20 - stop_distance)

trade_signal_executor()(
    enter_long=buy,
    exit_long=exit
)
```

---

## Execution Model

### Scripts Execute Per-Asset by Default

EpochScript strategies run in **time-series mode** - your entire script executes once for each asset in your configured universe. You don't specify which assets to analyze inside your script; the framework handles this automatically.

**Key Principle:** Universe configuration is **external** to your script code.

#### Antipattern: Using Non-Existent 'symbols' Parameter

<!-- snippet: type=error_example -->
```python
# Incorrect: - 'symbols' parameter does not exist
src = market_data_source(timeframe="1D", symbols=["AAPL", "MSFT", "GOOG"])
spx = indices(ticker="SPX", symbols=["SPX"])
```

**Error message:** `Unknown option 'symbols'`

**Why?** The `symbols` parameter doesn't exist on `market_data_source`, `indices`, or `common_indices`. Your script runs separately for each asset - the framework provides data for the current asset automatically.

#### Correct: Let Framework Handle Asset Iteration

<!-- snippet: type=complete_code, needs_data=single_asset, timeframe=1D, complexity=simple -->
```python
# Correct: - No symbols needed
src = market_data_source(timeframe="1D")
# Framework automatically provides data for current asset (AAPL, MSFT, etc.)

fast = ema(period=12)(src.c)
slow = ema(period=26)(src.c)
buy = crossover(fast, slow)

trade_signal_executor()(enter_long=buy)
```

**How it works:**
1. Universe configured at backtest/strategy level: ["AAPL", "MSFT", "GOOG"]
2. Framework runs your script 3 times:
   - Run 1: `market_data_source` fetches AAPL data
   - Run 2: `market_data_source` fetches MSFT data
   - Run 3: `market_data_source` fetches GOOG data
3. Each run is independent - separate trades, separate state

### Cross-Sectional Operations

To compare or rank assets **across the universe** at each timestamp, use transforms prefixed with `cs_`:

<!-- snippet: type=complete_code, needs_data=multi_asset, timeframe=1D, complexity=complex -->
```python
# Cross-sectional ranking strategy
src = market_data_source(timeframe="1D")

# Calculate 60-day momentum for this asset
returns = roc(period=60)(src.c)

# cs_momentum compares THIS asset against ALL assets in universe
# Returns percentile rank (0-100) at each timestamp
momentum_rank = cs_momentum()(returns)

# Select top 20% across entire universe
top_performers = top_k_percent(k=20)(momentum_rank)

trade_signal_executor()(enter_long=top_performers)
```

**Cross-sectional transforms:**
- `cs_momentum()` - Rank assets by input values (percentile 0-100)
- `cs_zscore()` - Normalize across assets (z-score at each timestamp)
- `top_k()` / `bottom_k()` - Select fixed count
- `top_k_percent()` / `bottom_k_percent()` - Select by percentage

**See also:** [Cross-Sectional Analysis](./concepts/cross-sectional) for detailed examples

### Comparing Against Benchmarks

For comparing each asset against a benchmark (e.g., SPX):

<!-- snippet: type=complete_code, needs_data=single_asset, timeframe=1D, complexity=medium -->
```python
# Each asset compared to SPX
src = market_data_source(timeframe="1D")  # Current asset
spx = common_indices(index="SPX")()       # Benchmark

# Relative strength
asset_ret = roc(period=20)(src.c)
spx_ret = roc(period=20)(spx.c)
outperforming = asset_ret > spx_ret

trade_signal_executor()(enter_long=outperforming)
```

---

## Python Features Not Supported

EpochScript looks like Python but is **NOT Python**. It's a domain-specific language for quantitative strategies with strict constraints.

### Control Flow Statements

**Not supported:**
- `if`/`elif`/`else` statements
- `for` loops
- `while` loops
- `def` function definitions
- `class` definitions
- `try`/`except` error handling

**Why?** EpochScript operates on time series data where every expression evaluates for all time points simultaneously. Control flow would break this model.

#### Wrong: If Statement

<!-- snippet: type=error_example -->
```python
# Incorrect: - if/else not allowed
if price > 100:
    signal = True
else:
    signal = False
```

**Error message:** `If statements not supported`

#### Correct: Boolean Expression or Ternary

<!-- snippet: type=complete_code, needs_data=single_asset, timeframe=1D, complexity=simple -->
```python
src = market_data_source(timeframe="1D")
price = src.c
rsi_val = rsi(period=14)(src.c)

# FIX 1: Direct boolean expression
signal = price > 100

# FIX 2: Ternary operator (for value selection)
signal2 = True if price > 100 else False

# FIX 3: conditional_select for multi-way branching
regime = conditional_select(
    rsi_val < 30, "oversold",
    rsi_val > 70, "overbought",
    "neutral"
)
```

### Variable Reassignment

Variables are **immutable** (single-assignment only). Once bound, they cannot be reassigned.

**Why?** Prevents temporal logic bugs and enables parallel evaluation.

#### Wrong: Reassignment

<!-- snippet: type=error_example, expected_error=! -->
```python
# Incorrect: - Cannot reassign variables
price = src.c
price = src.o  # ERROR!
```

**Error message:** `Variable 'price' already bound`

#### Correct: Unique Names

<!-- snippet: type=complete_code, needs_data=single_asset, timeframe=1D, complexity=simple -->
```python
src = market_data_source(timeframe="1D")

# FIX - Use distinct variable names
close_price = src.c
open_price = src.o
```

### Chained Comparisons

Python's chained comparison syntax is not supported.

**Why?** Ambiguous semantics for time series data.

#### Wrong: Chained Comparison

<!-- snippet: type=error_example -->
```python
# Incorrect: - Chained comparison not supported
valid = 30 < rsi_val < 70
```

**Error message:** `Chained comparison not supported`

#### Correct: Explicit Boolean Logic

<!-- snippet: type=complete_code, needs_data=single_asset, timeframe=1D, complexity=simple -->
```python
src = market_data_source(timeframe="1D")
rsi_val = rsi(period=14)(src.c)

# FIX - Break into explicit conditions
above_30 = rsi_val > 30
below_70 = rsi_val < 70
valid = above_30 and below_70

# Or inline:
valid2 = (rsi_val > 30) and (rsi_val < 70)
```

### Negative Indexing

Python's negative list indexing (`[-1]` for last element) is not supported.

**Why?** Lag semantics are forward-looking by default. Negative indices would be confusing.

#### Wrong: Negative Index

<!-- snippet: type=error_example -->
```python
# Incorrect: - Negative lag not allowed
yesterday = src.c[-1]  # Python lists allow this
```

**Error message:** `Negative lag indices not supported`

#### Correct: Positive Lag

<!-- snippet: type=complete_code, needs_data=single_asset, timeframe=1D, complexity=simple -->
```python
src = market_data_source(timeframe="1D")

# FIX - Use positive lag (historical lookback)
yesterday = src.c[1]  # 1 bar ago
last_week = src.c[5]  # 5 bars ago
```

### Zero Indexing

Python's `[0]` for current element is not needed.

#### Wrong: Zero Index

<!-- snippet: type=error_example -->
```python
# Incorrect: - Zero lag is redundant
current = src.c[0]
```

**Error message:** `invalid_lag_zero`

#### Correct: Omit Index for Current Value

<!-- snippet: type=complete_code, needs_data=single_asset, timeframe=1D, complexity=simple -->
```python
src = market_data_source(timeframe="1D")

# FIX - Omit index for current value
current = src.c
```

---

## Transform Usage Patterns

### Multi-Output Transform Access

Transforms that return multiple outputs (like `bbands`, `macd`, `stoch`) cannot be used directly in expressions without specifying which output.

**Why?** Ambiguous which output you want.

#### Anti-Pattern: Ambiguous Multi-Output

<!-- snippet: type=error_example -->
```python
# Incorrect: - Which output?
bb = bbands(period=20, stddev=2)(src.c)
signal = bb > 100  # bb has 3 outputs: lower, middle, upper
```

**Error message:** `ambiguous_multi_output: Cannot use multi-output transform in expression context`

#### Pattern 1: Attribute Access

<!-- snippet: type=complete_code, needs_data=single_asset, timeframe=1D, complexity=simple -->
```python
src = market_data_source(timeframe="1D")

# FIX - Access specific output by name
bb = bbands(period=20, stddev=2)(src.c)
signal = bb.bbands_upper > 100
resistance = bb.bbands_upper
support = bb.bbands_lower
```

#### Pattern 2: Tuple Unpacking

<!-- snippet: type=complete_code, needs_data=single_asset, timeframe=1D, complexity=simple -->
```python
src = market_data_source(timeframe="1D")

# FIX - Unpack all outputs
lower, middle, upper = bbands(period=20, stddev=2)(src.c)
signal = upper > 100

# Discard unwanted outputs with underscore
lower2, _, upper2 = bbands(period=20, stddev=2)(src.c)
```

### Tuple Unpacking Count Matching

When unpacking, variable count must match output count exactly (or use `_` for discards).

#### Wrong: Count Mismatch

<!-- snippet: type=error_example -->
```python
# Incorrect: - bbands has 3 outputs, but only 2 variables
lower, upper = bbands(period=20, stddev=2)(src.c)
```

**Error message:** `Tuple unpacking count mismatch: expected 3 values, got 2 variables`

#### Correct: Matching Count

<!-- snippet: type=complete_code, needs_data=single_asset, timeframe=1D, complexity=simple -->
```python
src = market_data_source(timeframe="1D")

# FIX 1: Match all outputs
lower, middle, upper = bbands(period=20, stddev=2)(src.c)

# FIX 2: Discard unwanted with underscore
lower2, _, upper2 = bbands(period=20, stddev=2)(src.c)

# FIX 3: Use attribute access instead
bb = bbands(period=20, stddev=2)(src.c)
lower3 = bb.bbands_lower
upper3 = bb.bbands_upper
```

### Only Unpack Transform Calls

You can only unpack direct transform constructor calls, not intermediate variables or expressions.

#### Wrong: Unpacking Non-Constructor

<!-- snippet: type=error_example -->
```python
# Incorrect: - Cannot unpack variable
bb = bbands(period=20, stddev=2)(src.c)
lower, middle, upper = bb  # Not a constructor call!
```

**Error message:** `Right-hand side must be a constructor call`

#### Correct: Unpack Constructor Directly

<!-- snippet: type=complete_code, needs_data=single_asset, timeframe=1D, complexity=simple -->
```python
src = market_data_source(timeframe="1D")

# FIX - Unpack the constructor call directly
lower, middle, upper = bbands(period=20, stddev=2)(src.c)
```

### event_marker Schema Pattern

> **See also:** [Event Marker System](./concepts/core-features#event-marker-system) for conceptual overview and usage examples

The `event_marker` transform uses `EventMarkerSchema` to configure how events are displayed.

#### Requirements:

1. **Color categories** in `color_map` must be: `Success` (green), `Warning` (yellow), `Error` (red), or `Info` (blue)
2. **Values** in color_map must match the actual column values being displayed
3. **select_key** (filter column) must be boolean
4. Use **SLOT references** for column_id to connect inputs

#### Wrong: Color Name Instead of Category

<!-- snippet: type=error_example, expected_error="Unknown color category" -->
```python
# Incorrect: - Using color names instead of categories
event_type = conditional_select(
    rsi_val < 30, "oversold",
    rsi_val > 70, "overbought")
any_event = (rsi_val < 30) | (rsi_val > 70)

event_marker(
    event_marker_schema=EventMarkerSchema(
        title="RSI Signals",
        select_key=SLOT0,
        icon=Signal,
        schemas=[
            CardColumnSchema(
                column_id=SLOT1,
                slot=PrimaryBadge,
                render_type=Badge,
                color_map={
                    "Green": ["oversold"],   # Wrong: Should be Success
                    "Red": ["overbought"]    # Wrong: Should be Error
                }
            )
        ]
    )
)(any_event, event_type)
```

**Error message:** `Unknown color category 'Green'`

#### Wrong: Value Not in color_map

<!-- snippet: type=error_example -->
```python
# Incorrect: - Column value doesn't match color_map values
event_type = "breakout"  # This string not in color_map
signal = src.c > upper

event_marker(
    event_marker_schema=EventMarkerSchema(
        title="Breakout Signals",
        select_key=SLOT0,
        icon=Signal,
        schemas=[
            CardColumnSchema(
                column_id=SLOT1,
                slot=PrimaryBadge,
                render_type=Badge,
                color_map={
                    Success: ["upper_break"]  # Expects "upper_break", not "breakout"
                }
            )
        ]
    )
)(signal, event_type)
```

**Error message:** `Value 'breakout' not found in color_map`

#### Wrong: Non-Boolean Filter

<!-- snippet: type=error_example, expected_error="Expected Boolean for select_key" -->
```python
# Incorrect: Passing non-boolean to select_key (SLOT0)
rsi_val = rsi(period=14)(src.c)

event_marker(
    event_marker_schema=EventMarkerSchema(
        title="RSI Values",
        select_key=SLOT0,  # Receives rsi_val (Decimal)
        icon=Signal,
        schemas=[
            CardColumnSchema(
                column_id=SLOT1,
                slot=Hero,
                render_type=Decimal
            )
        ]
    )
)(rsi_val, rsi_val)  # ERROR: SLOT0 is Decimal, not Boolean
```

**Error message:** `event_marker select_key expects Boolean input, got Decimal`

**Correct:**
<!-- snippet: type=complete_code, needs_data=single_asset, timeframe=1D, complexity=simple -->
```python
src = market_data_source(timeframe="1D")

# Create boolean filter condition
rsi_val = rsi(period=14)(src.c)
oversold = rsi_val < 30

event_marker(
    event_marker_schema=EventMarkerSchema(
        title="Oversold Events",
        select_key=SLOT0,  # Boolean filter
        icon=Signal,
        schemas=[
            CardColumnSchema(
                column_id=SLOT1,
                slot=Hero,
                render_type=Decimal
            )
        ]
    )
)(oversold, rsi_val)  # ✅ SLOT0 is Boolean
```

<Callout type="note">
The `select_key` parameter specifies which input (by SLOT number) acts as the boolean filter. Only rows where this column is True will show as event markers. Other columns display additional data about each event.
</Callout>

#### Correct: Complete EventMarker with Multiple Event Types

<!-- snippet: type=complete_code, needs_data=single_asset, timeframe=1D, complexity=medium -->
```python
# Correct: - Color-coded events with proper schema
src = market_data_source(timeframe="1D")
rsi_val = rsi(period=14)(src.c)
lower, _, upper = bbands(period=20, stddev=2)(src.c)

# Create event type string
event_type = conditional_select(
    rsi_val < 30, "oversold",
    rsi_val > 70, "overbought",
    src.c < lower, "support_break",
    src.c > upper, "resistance_break",
    crossover(src.c, upper), "crossover")

# Filter: any event occurred
any_event = (rsi_val < 30) | (rsi_val > 70) | (src.c < lower) | (src.c > upper) | crossover(src.c, upper)

event_marker(
    event_marker_schema=EventMarkerSchema(
        title="Trading Signals",
        select_key=SLOT0,  # Boolean filter
        icon=Signal,
        schemas=[
            CardColumnSchema(
                column_id=SLOT1,  # Event type string
                slot=PrimaryBadge,
                render_type=Badge,
                color_map={
                    Success: ["oversold", "support_break"],    # Green
                    Error: ["overbought", "resistance_break"],  # Red
                    Warning: ["neutral"],                       # Yellow
                    Info: ["crossover"]                         # Blue
                }
            ),
            CardColumnSchema(
                column_id=SLOT2,  # RSI value
                slot=Hero,
                render_type=Decimal
            )
        ]
    )
)(any_event, event_type, rsi_val)
```

---

## Timeframe and Session Formats

### Valid Timeframe Formats

EpochScript uses specific string formats for timeframes. Common abbreviations from Python or other languages won't work.

#### Valid Formats:

**Intraday:**
- `"1Min"`, `"5Min"`, `"15Min"`, `"30Min"`
- `"1H"`, `"2H"`, `"4H"`

**Daily and Higher:**
- `"1D"` (daily)
- `"1W"` (weekly)
- `"1M"` (monthly)

#### Wrong Formats:

<!-- snippet: type=error_example -->
```python
# Incorrect: - These don't work
daily = ema(period=50, timeframe="1Day")(src.c)   # Use "1D"
hourly = ema(period=20, timeframe="1Hour")(src.c)  # Use "1H"
minute = ema(period=10, timeframe="1Minute")(src.c) # Use "1Min"
```

**Error message:** `Invalid timeframe 'BAD_TF'`

#### Correct Formats:

<!-- snippet: type=complete_code, needs_data=single_asset, timeframe=1D, needs_intraday=true, complexity=medium -->
```python
# Correct:
daily = ema(period=50, timeframe="1D")(src.c)
hourly = ema(period=20, timeframe="1H")(src.c)
minute = ema(period=10, timeframe="1Min")(src.c)
```

### Valid Session Names

Session-based transforms use predefined session names.

#### Valid Sessions:

**Geographic:**
- `"Sydney"`, `"Tokyo"`, `"London"`, `"NewYork"`

**Kill Zones (ICT):**
- `"AsianKillZone"`
- `"LondonOpenKillZone"`
- `"NewYorkKillZone"`
- `"LondonCloseKillZone"`

#### Wrong Sessions:

<!-- snippet: type=error_example -->
```python
# Incorrect: - These don't exist
london = sessions(session_type="LondonKillZone")()  # Doesn't exist
ny = sessions(session_type="NY")()                   # Use "NewYork"
```

**Error message:** `Invalid session 'BAD_SESSION'`

#### Correct Sessions:

<!-- snippet: type=complete_code, needs_data=single_asset, needs_sessions=true, complexity=medium -->
```python
# Correct:
london = sessions(session_type="London")()
london_open_kz = sessions(session_type="LondonOpenKillZone")()
ny = sessions(session_type="NewYork")()
```

### Session-Based Transforms Require Intraday Data

Transforms like `sessions()`, `bar_gap()`, and `session_gap()` only work on intraday timeframes.

**Why?** Sessions are meaningful only for intraday data (London session, NY session, etc.). Daily bars don't have session boundaries.

#### Wrong: Sessions on Daily Data

<!-- snippet: type=error_example -->
```python
# Incorrect: - If strategy runs on daily (1D) timeframe
london = sessions(session_type="London")()
```

**Error message:** `Transform 'sessions' requires intraday data`

#### Correct: Sessions on Intraday Data

<!-- snippet: type=complete_code, needs_data=single_asset, needs_sessions=true, needs_intraday=true, complexity=medium -->
```python
# Correct: - Strategy configured for intraday (1Min, 5Min, 1H, etc.)
london = sessions(session_type="London")()
breakout = src.c > london.high
```

**Configuration:** Ensure your strategy/backtest is configured to run on intraday timeframe.

---

## Common Design Mistakes

### 1. Trying to Filter Data with If Statements

**Wrong approach:**
<!-- snippet: type=complete_code, needs_data=single_asset, needs_sessions=true, complexity=medium -->
```python
# Users often try this (Python thinking)
if london_session:
    signal = ema_cross
```

**Correct approach:**
<!-- snippet: type=complete_code, needs_data=single_asset, needs_sessions=true, needs_intraday=true, complexity=medium -->
```python
src = market_data_source(timeframe="5Min")

# Use boolean logic
fast = ema(period=12)(src.c)
slow = ema(period=26)(src.c)
ema_cross = crossover(fast, slow)
london = sessions(session_type="London")()
signal = ema_cross and london.active
```

### 2. Calling Output Attributes as Functions

**Wrong:**
<!-- snippet: type=complete_code, needs_data=single_asset, timeframe=1D, complexity=simple -->
```python
src = market_data_source(timeframe="1D")

# Trying to call output handle
bb = bbands(period=20, stddev=2)(src.c)
upper_val = bb.bbands_upper()  # Don't call it!
```

**Error message:** `Cannot call attribute as function`

**Correct:**
<!-- snippet: type=complete_code, needs_data=single_asset, timeframe=1D, complexity=simple -->
```python
src = market_data_source(timeframe="1D")

# Access attribute directly
bb = bbands(period=20, stddev=2)(src.c)
upper_val = bb.bbands_upper
```

### 3. Using Transform Name as Variable

**Confusing but allowed:**
<!-- snippet: type=complete_code, needs_data=single_asset, timeframe=1D, complexity=simple -->
```python
# Note: Legal but confusing
ema = market_data_source(timeframe="1D")
ema_20 = ema(period=20)(ema.c)  # 'ema' is the source, not the transform!
```

**Better:**
<!-- snippet: type=complete_code, needs_data=single_asset, timeframe=1D, complexity=simple -->
```python
# Clear naming
src = market_data_source(timeframe="1D")
ema_20 = ema(period=20)(src.c)
```

### 4. Forgetting Data Source

**Wrong:**
<!-- snippet: type=error_example -->
```python
# Where does 'c' come from?
ema_20 = ema(period=20)(c)
```

**Error message:** `Undefined variable 'c'`

**Correct:**
<!-- snippet: type=complete_code, needs_data=single_asset, timeframe=1D, complexity=simple -->
```python
# Explicitly fetch data source
src = market_data_source(timeframe="1D")
ema_20 = ema(period=20)(src.c)
```

### 5. Inconsistent Variable Naming in Unpacking

**Confusing:**
<!-- snippet: type=complete_code, needs_data=single_asset, timeframe=1D, complexity=simple -->
```python
src = market_data_source(timeframe="1D")

# Note: Legal but confusing naming
x, y, z = bbands(period=20, stddev=2)(src.c)
# Which is upper? Which is lower?
```

**Better:**
<!-- snippet: type=complete_code, needs_data=single_asset, timeframe=1D, complexity=simple -->
```python
src = market_data_source(timeframe="1D")

# Descriptive names
lower, middle, upper = bbands(period=20, stddev=2)(src.c)
# Or:
bb_lower, bb_middle, bb_upper = bbands(period=20, stddev=2)(src.c)
```

---

## Summary

**Key Principles:**

1. **Per-asset execution model** - Scripts run once per asset; universe configured externally
2. **One executor per strategy** - Required design pattern
3. **No Python control flow** - Use boolean logic and ternary operators
4. **Immutable variables** - Single assignment only
5. **Explicit multi-output access** - Unpack or use attribute access
6. **Strict format strings** - Timeframes and sessions have exact formats
7. **Intraday-only transforms** - Some transforms require intraday data
8. **Cross-sectional with cs_ prefix** - Use `cs_momentum()`, `cs_zscore()` for multi-asset ranking

**When in doubt:**
- Check [Error Reference](./error-reference) for specific error messages
- See [Language Fundamentals](./language) for detailed syntax rules
- Review [Core Concepts](./concepts) for working examples

---

**Related Documentation:**
- [Language Fundamentals](./language) - Syntax and types
- [Error Reference](./error-reference) - Troubleshooting specific errors
- [Core Concepts](./concepts) - Timeframes, sessions, cross-sectional, data sources
