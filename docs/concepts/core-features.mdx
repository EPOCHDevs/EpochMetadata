...
### Auto-Serialization in `color_map`

**Non-string values are automatically converted to strings** for `color_map` matching:
...
**Serialization rules:**
- `True` → `"True"`, `False` → `"False"` (Python capitalization)
- Integers: `42` → `"42"`
- Decimals: `3.14` → `"3.14"`
- Strings: unchanged

<Callout type="info">
**Match against the serialized value:** If your column contains boolean `True`, use `color_map={Info: [True]}`. If it contains the string `"true"`, use `color_map={Info: ["true"]}`.
</Callout>
