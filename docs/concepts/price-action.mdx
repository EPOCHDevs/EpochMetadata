...
swept_recently = liquidity_swept[1] or liquidity_swept[2] or liquidity_swept[3]
entry = swept_recently and at_order_block and (src.c > src.o)
```
...
bos_recently = (structure.bos == 1)[1] or (structure.bos == 1)[2] or (structure.bos == 1)[3] or (structure.bos == 1)[4] or (structure.bos == 1)[5]
...
golden_pocket = (retr.current_retracement >= 61.8) and (retr.current_retracement <= 78.6)
...
asc_tri = triangles(lookback=50, triangle_type="Ascending (Bullish)", r_squared_min=0.9)()
...
db = double_top_bottom(lookback=50, pattern_type="Double Bottom Only", similarity_tolerance=0.02)()
bullish_breakout = db.pattern_detected and (src.c > db.breakout_level)

# Double top (bearish reversal)
dt = double_top_bottom(lookback=50, pattern_type="Double Top Only", similarity_tolerance=0.02)()
...
pattern_event = ihs.pattern_detected or flag_pattern.bull_flag
gap_fill_event = gaps.gap_filled and (gaps.gap_size > 1.0)
liq_sweep_event = liq.swept != 0
gap_unfill_event = (gaps.gap_size > 1.0) and not gaps.gap_filled
any_event = pattern_event | gap_fill_event | liq_sweep_event | gap_unfill_event
