---
title: Error Reference
page_type: reference
layout: table
order: 1
category: Reference
description: Comprehensive troubleshooting guide with searchable error messages, causes, and fixes
parent: ./index.md
---

import { Callout } from 'fumadocs-ui/components/callout';

# Error Reference

Quick reference for EpochScript compilation and runtime errors. Each error includes the error message, cause, and fix example with links to detailed documentation.

<Callout type="tip">
**For detailed explanations and best practices:**
- [Types & Literals - Parameter Type Validation](./language/types#parameter-type-validation)
- [Design Guidelines](./design-guidelines) - Patterns and common mistakes
- [Language Limitations](./language/limitations) - Python features not supported
- [Core Concepts](./concepts) - Timeframes, sessions, cross-sectional
</Callout>

---


## Syntax Errors

### Variable Already Bound

**Error:** `Variable 'price' already bound`

**Cause:** Attempting to reassign a variable

<!-- snippet: type=error_example, expected_error="Cannot reassign" -->
```python
# Incorrect:
price = src.c
price = src.o  # ERROR: Cannot reassign

# FIX
close_price = src.c
open_price = src.o
```

**See also:** [Design Guidelines - Variable Reassignment](./design-guidelines.md#variable-reassignment)

---

### Undefined Variable

**Error:** `Undefined variable 'x'`

**Cause:** Using variable before definition

<!-- snippet: type=error_example -->
```python
# Incorrect:
signal = ema_val > 100
ema_val = ema(period=20)(src.c)  # Defined after use

# FIX
ema_val = ema(period=20)(src.c)
signal = ema_val > 100
```

---

### Chained Comparison Not Supported

**Error:** `Chained comparison not supported`

**Cause:** Using syntax like `a < b < c`

<!-- snippet: type=error_example -->
```python
# Incorrect:
valid = 30 < rsi_val < 70

# FIX
above_30 = rsi_val > 30
below_70 = rsi_val < 70
valid = above_30 and below_70
```

**See also:** [Design Guidelines - Chained Comparisons](./design-guidelines.md#chained-comparisons)

---

### Control Flow Not Allowed

**Error:** `If statements not supported`

**Cause:** Using `if`, `for`, `while`, `def`, `class`

<!-- snippet: type=error_example -->
```python
# Incorrect:
if price > 100:
    signal = True
else:
    signal = False

# FIX
signal = price > 100
# Or: signal = True if price > 100 else False
```

**See also:** [Design Guidelines - Control Flow Statements](./design-guidelines.md#control-flow-statements)

---

## Type Errors

### Incompatible Type Cast

**Error:** `incompatible_type_cast`

**Cause:** Trying to mix incompatible types (e.g., String + Number)

<!-- snippet: type=error_example -->
```python
# Incorrect:
label = "PRICE"
result = label + 100  # Cannot add string to number

# FIX
value = 100
result = value + 100
```

---

### Type Mismatch for Option

**Error:** `Type mismatch for option 'period': expected Integer, got String`

**Cause:** Wrong type for transform parameter

<!-- snippet: type=error_example -->
```python
# Incorrect:
ema_val = ema(period="20")(src.c)  # String instead of Integer

# FIX
ema_val = ema(period=20)(src.c)
```

**See also:** [Types & Literals - Parameter Type Validation](./language/types.md#parameter-type-validation)

---

### Boolean Operation on Non-Boolean

**Error:** `Expected Boolean type for logical operation`

**Cause:** Using `and`/`or`/`not` with non-boolean values incorrectly

<!-- snippet: type=error_example -->
```python
# Incorrect: (if not auto-cast)
value = 100
result = value and True  # May fail if no auto-cast

# FIX
is_positive = value > 0
result = is_positive and True
```

---

## Transform Errors

**See also:** [Core Concepts](./concepts) for timeframes, sessions, and cross-sectional capabilities.

### Missing Required Option

**Error:** `Missing required option 'period'`

**Cause:** Transform requires an option that wasn't provided

<!-- snippet: type=error_example -->
```python
# Incorrect:
ema_val = ema()(src.c)  # Missing period

# FIX
ema_val = ema(period=20)(src.c)
```

---

### Unknown Option

**Error:** `Unknown option 'periood'`

**Cause:** Typo in option name

<!-- snippet: type=error_example -->
```python
# Incorrect:
ema_val = ema(periood=20)(src.c)  # Typo

# FIX
ema_val = ema(period=20)(src.c)
```

---

### Unknown Handle

**Error:** `Unknown handle 'result' for transform 'market_data_source'`

**Cause:** Accessing wrong output name

<!-- snippet: type=error_example -->
```python
# Incorrect:
src = market_data_source(timeframe="1D")
price = src.result  # market_data_source has no 'result'

# FIX
price = src.c  # Use correct handle: c, h, l, o, v
```

---

### Transform Not Found

**Error:** `Unknown transform 'ema_20'`

**Cause:** Typo in transform name or transform doesn't exist

<!-- snippet: type=error_example -->
```python
# Incorrect:
ema_val = ema_20()(src.c)  # No such transform

# FIX
ema_val = ema(period=20)(src.c)
```

---

### Wrong Input Count

**Error:** `Expected 2 inputs, got 1`

**Cause:** Transform requires specific number of inputs

<!-- snippet: type=error_example -->
```python
# Incorrect:
cross = crossover()(fast)  # Needs 2 inputs

# FIX
cross = crossover()(fast, slow)
```

---

### Wrong Input Type

**Error:** `Expected Decimal input, got Boolean`

**Cause:** Passing wrong type to transform input

<!-- snippet: type=error_example -->
```python
# Incorrect:
ema_val = ema(period=20)(is_bullish)  # Boolean, not Decimal

# FIX
ema_val = ema(period=20)(src.c)  # Decimal
```

---

## Lag Operator Errors

### Invalid Lag Zero

**Error:** `invalid_lag_zero`

**Cause:** Using `[0]` as lag index

<!-- snippet: type=error_example -->
```python
# Incorrect:
current = src.c[0]

# FIX
current = src.c  # Omit index for current value
```

**See also:** [Design Guidelines - Zero Indexing](./design-guidelines.md#zero-indexing)

---

### Invalid Lag Float

**Error:** `invalid_lag_float`

**Cause:** Using float as lag index

<!-- snippet: type=error_example -->
```python
# Incorrect:
lag = src.c[1.5]

# FIX
lag = src.c[2]  # Use integer
```

---

### Negative Lag Not Allowed

**Error:** `Negative lag indices not supported`

**Cause:** Using negative index (forward-looking)

<!-- snippet: type=error_example -->
```python
# Incorrect:
future = src.c[-1]

# FIX (for research only)
# Use forward_returns transform
future_ret = forward_returns(period=1)(src.c)
```

**See also:** [Design Guidelines - Negative Indexing](./design-guidelines.md#negative-indexing)

---

## Multi-Output Errors

### Ambiguous Multi-Output

**Error:** `ambiguous_multi_output`

**Cause:** Using multi-output transform in expression without specifying which output

<!-- snippet: type=error_example -->
```python
# Incorrect:
bb = bbands(period=20, stddev=2)(src.c)
signal = bb > 100  # Which output? (lower, middle, or upper?)

# FIX - Option 1: Access specific handle
bb = bbands(period=20, stddev=2)(src.c)
signal = bb.bbands_upper > 100

# FIX - Option 2: Tuple unpacking
lower, middle, upper = bbands(period=20, stddev=2)(src.c)
signal = upper > 100
```

**See also:** [Design Guidelines - Multi-Output Transform Access](./design-guidelines.md#multi-output-transform-access)

---

### Tuple Unpacking Count Mismatch

**Error:** `Tuple unpacking count mismatch: expected 3, got 2`

**Cause:** Number of variables doesn't match number of outputs

<!-- snippet: type=error_example -->
```python
# Incorrect:
lower, upper = bbands(period=20, stddev=2)(src.c)  # 3 outputs, 2 variables

# FIX - Option 1: Match count
lower, middle, upper = bbands(period=20, stddev=2)(src.c)

# FIX - Option 2: Discard with underscore
lower, _, upper = bbands(period=20, stddev=2)(src.c)
```

**See also:** [Design Guidelines - Tuple Unpacking Count Matching](./design-guidelines.md#tuple-unpacking-count-matching)

---

### Right-Hand Side Must Be Constructor Call

**Error:** `Right-hand side of tuple unpacking must be a constructor call`

**Cause:** Trying to unpack something that isn't a transform call

<!-- snippet: type=error_example -->
```python
# Incorrect:
bb = bbands(period=20, stddev=2)(src.c)
lower, middle, upper = bb  # Cannot unpack after assignment

# FIX
lower, middle, upper = bbands(period=20, stddev=2)(src.c)
```

**See also:** [Design Guidelines - Only Unpack Transform Calls](./design-guidelines.md#only-unpack-transform-calls)

---

### Calling Attribute as Function

**Error:** `Cannot call attribute as function`

**Cause:** Trying to call an output handle

<!-- snippet: type=error_example -->
```python
# Incorrect:
bb_upper = bbands(period=20, stddev=2).bbands_upper(src.c)

# FIX
bb = bbands(period=20, stddev=2)(src.c)
bb_upper = bb.bbands_upper  # Access, don't call
```

**See also:** [Design Guidelines - Common Design Mistakes](./design-guidelines.md#2-calling-output-attributes-as-functions)

---

## TimeFrame/Session Errors

### Missing Required Timeframe

**Error:** `Transform 'market_data_source' requires a timeframe`

**Cause:** Transform with `requiresTimeFrame=true` has no explicit timeframe and no inputs to inherit from

<!-- snippet: type=error_example -->
```python
# Incorrect:
src = market_data_source()  # Missing timeframe
gaps = session_gap(fill_percent=100)()  # No timeframe, no inputs

# FIX - Option 1: Set explicit timeframe
src = market_data_source(timeframe="1D")
gaps = session_gap(fill_percent=100, timeframe="1Min")()

# FIX - Option 2: Connect to input with timeframe
src = market_data_source(timeframe="1D")
gaps = session_gap(fill_percent=100)(src.c)  # Inherits "1D" from src
```

**Transforms requiring explicit timeframes:**
- Data sources: `market_data_source`, `vwap`, `trade_count` (no inputs)
- Calendar effects: `day_of_week`, `week_of_month`, `holiday` (no inputs)
- Chart patterns with no inputs: `swing_highs_lows`, `head_and_shoulders`

**See also:** [Timeframes - Timeframe Requirements](./concepts/timeframes.md#timeframe-requirements)

---

### Invalid TimeFrame

**Error:** `Invalid timeframe 'BAD_TF'`

**Cause:** Using unsupported timeframe string

<!-- snippet: type=error_example -->
```python
# Incorrect:
daily = ema(period=50, timeframe="BAD_TF")(src.c)
daily = ema(period=50, timeframe="1Day")(src.c)  # Wrong format

# FIX
daily = ema(period=50, timeframe="1D")(src.c)
hourly = ema(period=20, timeframe="1H")(src.c)
```

**Valid formats:** `"1Min"`, `"2Min"`, `"3Min"`, `"5Min"`, `"10Min"`, `"15Min"`, `"30Min"`, `"45Min"`, `"1H"`, `"2H"`, `"3H"`, `"4H"`, `"1D"`, `"1W-MON"`, `"1W-FRI"`, `"1W-SUN"`, `"1ME"`, `"1MS"`, `"1QE"`, `"1QS"`, `"1YE"`, `"1YS"`

**See also:** [Timeframes - Standard Timeframes](./concepts/timeframes.md#standard-timeframes)

---

### Invalid Session

**Error:** `Invalid session 'BAD_SESSION'`

**Cause:** Using unsupported session name

<!-- snippet: type=error_example -->
```python
# Incorrect:
london_ema = ema(period=20, session="BAD_SESSION")(src.c)
london_ema = ema(period=20, session="LondonKillZone")(src.c)  # Doesn't exist

# FIX
london_ema = ema(period=20, session="London")(src.c)
lkz_ema = ema(period=20, session="LondonOpenKillZone")(src.c)
```

**Valid sessions:** `"Sydney"`, `"Tokyo"`, `"London"`, `"NewYork"`, `"AsianKillZone"`, `"LondonOpenKillZone"`, `"NewYorkKillZone"`, `"LondonCloseKillZone"`

**See also:** [Design Guidelines - Valid Session Names](./design-guidelines.md#valid-session-names)

---

### Intraday-Only Transform on Daily Data

**Error:** `Transform 'sessions' requires intraday data`

**Cause:** Using session-based transform on daily or higher timeframe

<!-- snippet: type=error_example -->
```python
# Incorrect: (if running on daily data)
london = sessions(session_type="London")()

# FIX
# Use intraday timeframe (1Min, 5Min, 15Min, etc.)
# Configure strategy to run on intraday data
```

**See also:** [Design Guidelines - Session-Based Transforms Require Intraday Data](./design-guidelines.md#session-based-transforms-require-intraday-data)

---

### Unknown Option 'symbols' for Data Sources

**Error:** `Unknown option 'symbols'`

**Cause:** Attempting to use non-existent `symbols` parameter on data source transforms

**Common Antipattern:**
<!-- snippet: type=error_example -->
```python
# Incorrect: - These transforms do NOT have a 'symbols' parameter
src = market_data_source(timeframe="1Min", symbols=["SPY-Stocks"])
src = market_data_source(timeframe="1D", symbols=["AAPL", "MSFT", "GOOG"])
spx = indices(ticker="SPX", symbols=["SPX", "NDX"])  # WRONG
benchmark = common_indices(index="SPX", symbols=["SPX"])  # WRONG
```

**Error message:** `Unknown option 'symbols' for transform 'market_data_source'` (or `'indices'`, `'common_indices'`)

**Why This Is Wrong:**

EpochScript strategies execute **per-asset by default** in time-series mode. The framework runs your entire script once for each asset in your configured universe. You don't specify which assets to analyze inside your script - that's configured at the strategy/backtest level.

**Key Concepts:**

1. **Time-Series Execution (Default):** Your script runs independently for each asset in the universe. `market_data_source(timeframe="1D")` automatically fetches data for whatever asset the script is currently processing.

2. **Cross-Sectional Operations:** To compare or rank across multiple assets at once, use transforms prefixed with `cs_` (e.g., `cs_momentum()`, `cs_zscore()`, `top_k()`). These operate across all assets at each timestamp.

**Correct Patterns:**

<!-- snippet: type=complete_code, needs_data=single_asset, timeframe=1D, complexity=simple -->
```python
# Pattern 1: Single-asset time-series strategy
# (Script runs once per asset in configured universe)
src = market_data_source(timeframe="1D")
fast = ema(period=12)(src.c)
slow = ema(period=26)(src.c)
buy = crossover(fast, slow)

trade_signal_executor()(enter_long=buy)
```

<!-- snippet: type=complete_code, needs_data=multi_asset, timeframe=1D, complexity=complex -->
```python
# Pattern 2: Cross-sectional strategy
# (Script runs per-asset, but cs_ transforms compare across all assets)
src = market_data_source(timeframe="1D")

# Calculate 60-day returns for each asset
returns = roc(period=60)(src.c)

# cs_momentum ranks THIS asset against ALL other assets in universe
# Returns percentile rank (0-100) at each timestamp
momentum_rank = cs_momentum()(returns)

# Select top 20% of assets by momentum
top_performers = top_k_percent(k=20)(momentum_rank)

trade_signal_executor()(enter_long=top_performers)
```

<!-- snippet: type=complete_code, needs_data=single_asset, timeframe=1D, complexity=medium -->
```python
# Pattern 3: Comparing against a benchmark index (e.g., SPX)
# Each asset's data automatically provided by market_data_source
src = market_data_source(timeframe="1D")

# Fetch benchmark separately (not in universe, just for comparison)
spx = common_indices(index="SPX")()
# Or: spx = indices(ticker="SPX")()

# Compare asset performance vs SPX
asset_return = roc(period=20)(src.c)
spx_return = roc(period=20)(spx.c)

# Relative strength: asset outperforming SPX
outperforming = asset_return > spx_return

trade_signal_executor()(enter_long=outperforming)
```

**How Universe Selection Works:**

The asset universe is configured **outside your script**:
- In backtests: Configure universe in backtest settings (e.g., "S&P 500", custom list)
- In strategies: Universe defined at strategy configuration level
- Your script logic is the **same** regardless of universe size

**See also:**
- [Cross-Sectional Analysis](./concepts/cross-sectional) - Using `cs_` transforms for multi-asset strategies
- [Design Guidelines - Execution Model](./design-guidelines.md#execution-model) - Per-asset vs cross-sectional execution

---

## Reporting Errors

### event_marker: Invalid Color Category

**Error:** `Unknown color category 'Green'`

**Cause:** Using wrong color name (must be Success, Warning, Error, or Info)

<!-- snippet: type=error_example -->
```python
# Incorrect:
event_type = conditional_select(
    oversold, "oversold",
    overbought, "overbought")
any_event = oversold | overbought

event_marker(
    event_marker_schema=EventMarkerSchema(
        title="RSI Events",
        select_key=SLOT0,
        icon=Signal,
        schemas=[
            CardColumnSchema(
                column_id=SLOT1,
                slot=PrimaryBadge,
                render_type=Badge,
                color_map={
                    Green: ["oversold"],     # Wrong: 'Green' not valid
                    Red: ["overbought"]      # Wrong: 'Red' not valid
                }
            )
        ]
    )
)(any_event, event_type)

# FIX
event_marker(
    event_marker_schema=EventMarkerSchema(
        title="RSI Events",
        select_key=SLOT0,
        icon=Signal,
        schemas=[
            CardColumnSchema(
                column_id=SLOT1,
                slot=PrimaryBadge,
                render_type=Badge,
                color_map={
                    Success: ["oversold"],   # Green color
                    Error: ["overbought"]    # Red color
                }
            )
        ]
    )
)(any_event, event_type)
```

**Valid color categories:** `Success` (green), `Warning` (yellow), `Error` (red), `Info` (blue)

**See also:** [Design Guidelines - event_marker Color Map Pattern](./design-guidelines.md#event_marker-color-map-pattern)

---

### event_marker: Column Value Not in color_map

**Error:** `Value 'breakout' not found in color_map`

**Cause:** Column value doesn't match any values in color_map arrays

<!-- snippet: type=error_example -->
```python
# Incorrect:
event_type = "breakout"  # This value not in color_map
any_event = src.c > upper

event_marker(
    event_marker_schema=EventMarkerSchema(
        title="Breakout Signals",
        select_key=SLOT0,
        icon=Signal,
        schemas=[
            CardColumnSchema(
                column_id=SLOT1,
                slot=PrimaryBadge,
                render_type=Badge,
                color_map={
                    Success: ["upper_break"],  # Expects "upper_break", not "breakout"
                    Error: ["lower_break"]
                }
            )
        ]
    )
)(any_event, event_type)

# FIX - Option 1: Match the value in color_map
event_type = "upper_break"  # Matches color_map value
event_marker(
    event_marker_schema=EventMarkerSchema(
        title="Breakout Signals",
        select_key=SLOT0,
        icon=Signal,
        schemas=[
            CardColumnSchema(
                column_id=SLOT1,
                slot=PrimaryBadge,
                render_type=Badge,
                color_map={
                    Success: ["upper_break"],
                    Error: ["lower_break"]
                }
            )
        ]
    )
)(any_event, event_type)

# FIX - Option 2: Update color_map to match value
event_type = "breakout"
event_marker(
    event_marker_schema=EventMarkerSchema(
        title="Breakout Signals",
        select_key=SLOT0,
        icon=Signal,
        schemas=[
            CardColumnSchema(
                column_id=SLOT1,
                slot=PrimaryBadge,
                render_type=Badge,
                color_map={
                    Success: ["breakout"]  # Now matches
                }
            )
        ]
    )
)(any_event, event_type)
```

---

### event_marker: Non-Boolean Filter Input

**Error:** `Expected Boolean input for select_key, got Decimal`

**Cause:** The select_key (filter column) must be boolean

<!-- snippet: type=error_example -->
```python
# Incorrect:
event_marker(
    event_marker_schema=EventMarkerSchema(
        title="Price Events",
        select_key=SLOT0,  # This slot receives src.c (Decimal)
        icon=Signal,
        schemas=[
            CardColumnSchema(
                column_id=SLOT1,
                slot=Hero,
                render_type=Decimal
            )
        ]
    )
)(src.c, src.c)  # SLOT0 is Decimal, not Boolean

# FIX
high_price_event = src.c > 100  # Boolean condition
event_marker(
    event_marker_schema=EventMarkerSchema(
        title="High Price Events",
        select_key=SLOT0,  # Now receives boolean
        icon=Signal,
        schemas=[
            CardColumnSchema(
                column_id=SLOT1,
                slot=Hero,
                render_type=Decimal
            )
        ]
    )
)(high_price_event, src.c)  # SLOT0 is Boolean filter
```

**See also:** [Research Reports](./concepts/research) - Gap Report and Table Report documentation

---

## Quick Troubleshooting

### Compilation Errors

| Symptom | Likely Cause | Fix |
|---------|-------------|-----|
| `Variable 'x' already bound` | Reassignment | Use different variable name |
| `Undefined variable 'x'` | Used before definition | Move definition above usage |
| `Chained comparison` | `a < b < c` syntax | Break into `(a < b) and (b < c)` |
| `If statements not supported` | Control flow | Use ternary or `conditional_select` |
| `Unknown option 'x'` | Typo in option name | Check spelling |
| `Missing required option 'x'` | Required parameter missing | Add the option |

### Runtime Errors

| Symptom | Likely Cause | Fix |
|---------|-------------|-----|
| `incompatible_type_cast` | String + Number | Check types |
| `Unknown handle 'x'` | Wrong output name | Check transform outputs |
| `invalid_lag_zero` | Using `[0]` | Omit index |
| `invalid_lag_float` | Using `[1.5]` | Use integer |
| `ambiguous_multi_output` | Multi-output in expression | Access specific handle |
| `Tuple unpacking count mismatch` | Wrong number of variables | Match output count |

### Strategy Errors

| Symptom | Likely Cause | Fix |
|---------|-------------|-----|
| `No trade_signal_executor` | Missing executor | See [Design Guidelines](./design-guidelines.md#every-trading-strategy-must-have-exactly-one-executor) |
| `Multiple executors` | Two `trade_signal_executor` calls | See [Design Guidelines](./design-guidelines.md#anti-pattern-multiple-executors) |
| `Transform requires a timeframe` | Data source or no-input transform | Add `timeframe="1D"` parameter |
| `Invalid timeframe` | Wrong format | Use `"1D"`, `"1H"`, `"1W-MON"`, etc. |
| `Invalid session` | Wrong name | Use valid session names |
| `Unknown option 'symbols'` | Using non-existent parameter | Remove `symbols=` - universe is configured externally |

---

## Debugging Steps

### 1. Read the Error Message

Error messages tell you:
- **What** went wrong
- **Where** it occurred (line number)
- **Why** it failed

### 2. Check Variable Names

<!-- snippet: type=error_example -->
```python
src = market_data_source(timeframe="1D")

# Common typos
ema_val = ema(period=20)(src.c)
signal = emv_val > 100  # Typo: emv_val vs ema_val
```

### 3. Verify Transform Names and Options

<!-- snippet: type=complete_code, needs_data=single_asset, timeframe=1D, complexity=simple -->
```python
src = market_data_source(timeframe="1D")

# Check spelling
rsi_val = rsi(period=14)(src.c)  # TRUE
rsi_val2 = rsi(periood=14)(src.c)  # FALSE Typo

# Check required options
ema_val = ema()(src.c)  # FALSE Missing period
ema_val2 = ema(period=20)(src.c)  # TRUE
```

### 4. Check Output Handles

<!-- snippet: type=error_example -->
```python
# Wrong handle
src = market_data_source(timeframe="1D")
price = src.result  # FALSE No 'result' handle

# Correct handles
price = src.c  # Close
high = src.h   # High
```

### 5. Verify Input/Output Counts

<!-- snippet: type=complete_code, needs_data=single_asset, timeframe=1D, complexity=simple -->
```python
src = market_data_source(timeframe="1D")
fast = ema(period=12)(src.c)
slow = ema(period=26)(src.c)

# Crossover needs 2 inputs
cross = crossover()(fast)  # FALSE Only 1 input
cross2 = crossover()(fast, slow)  # TRUE

# Bollinger Bands returns 3 outputs
lower, upper = bbands(period=20, stddev=2)(src.c)  # FALSE Only 2 variables
lower2, middle2, upper2 = bbands(period=20, stddev=2)(src.c)  # TRUE
```

### 6. Check Data Types

<!-- snippet: type=error_example -->
```python
# Wrong type
ema_val = ema(period="20")(src.c)  # FALSE String
ema_val = ema(period=20)(src.c)    # Integer

# Type compatibility
label = "PRICE"
result = label + 100  # FALSE String + Number
```

---

## Common Patterns for Fixes

### Pattern 1: Variable Reassignment

<!-- snippet: type=error_example, expected_error="# Solution" -->
```python
# Problem
x = src.c
x = src.o  # ERROR

# Solution
close = src.c
open = src.o
```

### Pattern 2: Chained Comparison

<!-- snippet: type=error_example, expected_error="# Solution" -->
```python
# Problem
valid = 30 < rsi < 70  # ERROR

# Solution
valid = (rsi > 30) and (rsi < 70)
```

### Pattern 3: Multi-Output Access

<!-- snippet: type=error_example, expected_error="# Solution 1: Specific handle" -->
```python
# Problem
bb = bbands(period=20, stddev=2)(src.c)
signal = bb > 100  # ERROR

# Solution 1: Specific handle
signal = bb.bbands_upper > 100

# Solution 2: Tuple unpacking
lower, middle, upper = bbands(period=20, stddev=2)(src.c)
signal = upper > 100
```

### Pattern 4: Missing Executor

**See:** [Design Guidelines - Strategy Executor Requirements](./design-guidelines.md#strategy-executor-requirements)

### Pattern 5: Conditional Logic

<!-- snippet: type=error_example, expected_error="x = 1" -->
```python
# Problem
if condition:  # ERROR
    x = 1

# Solution 1: Ternary
x = 1 if condition else 0

# Solution 2: Direct boolean
x = condition  # If you just need True/False
```

---

## Getting More Help

If error persists:

1. **Check design guidelines**: Review [Design Guidelines](./design-guidelines) for required patterns
2. **Check transform catalog**: Use the UI Transform Browser to verify transform name, options, inputs, outputs
3. **Simplify**: Remove parts until error disappears, then add back
4. **Test incrementally**: Build up strategy step-by-step

---

**Related Documentation:**
- [Design Guidelines](./design-guidelines) - Required design patterns and constraints
- [Language Fundamentals](./language) - Syntax and types
- [Core Concepts](./concepts) - Timeframes, sessions, cross-sectional, data sources
