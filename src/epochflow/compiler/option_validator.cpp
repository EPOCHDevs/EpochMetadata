...
namespace epoch_stratifyx::epochflow
{
    ...
    epoch_metadata::MetaDataOptionDefinition::T OptionValidator::ParseOptionByMetadata(
        ...
        const epoch_metadata::transforms::TransformsMetaData& comp_meta)
    {
        ...
        case MetaType::CardSchema:
        {
            ...
            if (std::holds_alternative<epoch_metadata::CardSchemaFilter>(raw_value) ||
                std::holds_alternative<epoch_metadata::CardSchemaSQL>(raw_value))
            {
                return raw_value;
            }
            ...
            if (sql_result)
            {
                ...
                return epoch_metadata::MetaDataOptionDefinition::T{card_schema_sql};
            }
        ...
        case MetaType::SqlStatement:
        {
            ...
            if (std::holds_alternative<epoch_metadata::SqlStatement>(raw_value))
            {
                return raw_value;
            }
            ...
            try
            {
                ...
                epoch_metadata::SqlStatement sql_stmt{sql_str};
                int num_outputs = static_cast<int>(comp_meta.outputs.size());
                sql_stmt.Validate(num_outputs);
                return epoch_metadata::MetaDataOptionDefinition::T{sql_stmt};
            }
            catch (const std::exception& e)
            {
                ThrowError(
        ...
        case MetaType::Time:
        {
            ...
            if (std::holds_alternative<std::string>(raw_value))
            {
                ...
                try
                {
                    epoch_frame::Time time = epoch_metadata::TimeFromString(time_str);
        ...
        {
            // Expect a Sequence
            if (!std::holds_alternative<epoch_metadata::Sequence>(raw_value))
            {
                ThrowError(
            ...
            const auto& sequence = std::get<epoch_metadata::Sequence>(raw_value);
