...
namespace epoch_metadata::transforms {
    ...
    TransformsMetaData MakeMathTransformMetaData(std::string const &id,
        ...
        metadata.id = id;
        metadata.name = name;
        metadata.type = epoch_core::TransformType::Simple; // Adjust type as necessary
        metadata.isCrossSectional = false;
        metadata.desc = "";
    ...
    std::vector<TransformsMetaData> MakeScalarMetaData() {
        ...
        metadataList.emplace_back(TransformsMetaData{.id="number",
             .name="Number",
             .options = {
    ...
    std::vector<TransformsMetaData> MakeTulipIndicators() {
        ...
        auto getType = [](const int type, std::string const& name) -> epoch_core::TransformType {
            ...
            switch (type) {
                ...
                case TI_TYPE_MATH:
                    return overlayMath.contains(name) ? epoch_core::TransformType::Overlay : epoch_core::TransformType::MathFunction;
                case TI_TYPE_SIMPLE:
        ...
        std::ranges::transform(
            ...
            allIndicators.begin(), [&](const ti_indicator_info &tiIndicatorInfo) {
              ...
              return TransformsMetaData{
                  ...
                  .type = getType(tiIndicatorInfo.type, tiIndicatorInfo.name),
                  .isCrossSectional = false,
    ...
    std::vector<TransformsMetaData> MakeTulipCandles() {
        ...
        auto makeCandleOutputs = [](tc_candle_info const &info) {
            ...
            return std::vector<IOMetaData>{{.type = epoch_core::IODataType::Boolean,
                                            .id = info.name,
