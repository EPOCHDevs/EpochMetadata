...
std::vector<size_t> argrelmin(std::span<T> const &data, int order = 1) {
  return nonzero(boolrelextrema(data, epoch_metadata::math::less, order));
}

template <class T>
std::vector<size_t> argrelmax(std::span<T> const &data, int order = 1) {
  return nonzero(boolrelextrema(data, epoch_metadata::math::greater, order));
}
