...
namespace epoch_flow::runtime {
  std::optional<epoch_metadata::TimeFrame> TimeframeResolutionCache::ResolveTimeframe(
    ...
    std::optional<epoch_metadata::TimeFrame> resolvedTimeframe;

    // If we have inputs, resolve from them
    if (!inputIds.empty()) {
      std::vector<epoch_metadata::TimeFrame> inputTimeframes;
  ...
  void TransformManager::BuildTransformManager(
    ...
    for (auto &algorithm : algorithms) {
      ...
      this->Insert(
          epoch_metadata::TransformDefinition{algorithm, resolvedTimeframe});
      if (algorithm.type == epoch_metadata::transforms::TRADE_SIGNAL_EXECUTOR_ID) {
