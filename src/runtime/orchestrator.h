...
#pragma once
#include <epoch_metadata/runtime/iorchestrator.h>
...
// Removed: #include <model/common/constants.h> - not needed
#include <tbb/flow_graph.h>

// TODO: epoch_folio dependency removed - not currently used
// Forward declaration for IReporter interface
...
namespace epoch_flow::runtime {
    ...
    public:
        ...
        void
        RegisterTransform(std::unique_ptr<epoch_metadata::transform::ITransformBase> transform);
    ...
    private:
        ...
        std::vector<std::unique_ptr<epoch_metadata::transform::ITransformBase>>
            m_transforms;
        ...
        TransformNodePtr
        CreateTransformNode(epoch_metadata::transform::ITransformBase& transform);

        std::vector<DataFlowRuntimeOrchestrator::TransformExecutionNode *>
        ResolveInputDependencies(const epoch_metadata::strategy::InputMapping &inputs) const;

        // Helper to cache reports from reporter transforms
        void CacheReportFromTransform(const epoch_metadata::transform::ITransformBase& transform) const;

        // Helper to cache selectors from selector transforms
        void CacheSelectorFromTransform(const epoch_metadata::transform::ITransformBase& transform) const;
    };
