...
/**
 * @brief Builder for ao PlotKind (Awesome Oscillator)
 */
class AOBuilder : public IPlotKindBuilder {
public:
  ...
  void Validate(
    ...
    if (outputs.empty()) {
      throw std::runtime_error("AO transform has no outputs");
    }
    ...
    if (outputs.size() > 1) {
      if (!cfg.ContainsOutputId("result") && !cfg.ContainsOutputId("value")) {
        throw std::runtime_error(
          "AO transform with multiple outputs must have 'result' or 'value' output"
        );
      }
    }
  }
