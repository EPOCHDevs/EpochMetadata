 ...
 */
class PanelLineBuilder : public IPlotKindBuilder {
public:
  ...
  void Validate(
    ...
    if (outputs.empty()) {
      throw std::runtime_error("PanelLine transform has no outputs");
    }
    ...
    if (outputs.size() > 1) {
      if (!cfg.ContainsOutputId("result") && !cfg.ContainsOutputId("value")) {
        throw std::runtime_error(
          "PanelLine transform with multiple outputs must have 'result' or 'value' output"
        );
      }
    }
  }
