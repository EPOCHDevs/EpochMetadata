...
public:
  explicit BooleanCardReport(epoch_metadata::transform::TransformConfiguration config)
    ...
    return {
      ...
      .options = {
        {.id = "agg",
         ...
         .defaultValue = epoch_metadata::MetaDataOptionDefinition{"any"},
         .isRequired = false,
         .selectOption = {
                          {"All", "all"},
                          {"Any", "any"}
      ...
      .inputs = {
        {epoch_core::IODataType::Boolean, epoch_metadata::ARG}
      },
